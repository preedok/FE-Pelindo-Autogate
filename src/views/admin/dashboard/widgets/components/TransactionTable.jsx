...
const TransactionTable = ({ id, name, transactionType }) => {
    // transaction
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            console.log('response data transaksi', response.data)
    ...
    const fetchData = async () => {
        const token = localStorage.getItem('token');
        const endpoint = transactionType === 'import' ? 'I' : 'E';
        try {
            setLoading(true);
            const response = await api.get(`/Container?EI=${endpoint}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    }, [autoRefresh, fetchData, searchQuery, transactionType, initialDataLoaded]);
    const rows = data.map((data, index) => {
        ...
        return createData(
            ...
            movement,
            data.releaseNote ? (data.holdStatus === 'R' || data.holdStatus === 'N' ? data.releaseNote : (data.holdStatus === 'H' ? data.holdNote : '-')) : '-',
            data.releaseBy ? data.releaseBy : data.holdBy ? data.holdBy : "-",
    ...
    return (
        <>
            <div className="flex justify-between flex-col sm:flex-row gap-4 w-full">
                <div className='flex gap-3'>
                    <Button
                        ...
                        style={{
                            fontSize: '8px',
                            fontWeight: '700',
                            backgroundColor: autoRefresh ? '#65B741' : 'red'
                        }}
                        onClick={toggleAutoRefresh}
                    >
                        {autoRefresh ? <span><AutorenewIcon fontSize='small' style={{
                            animation: loading ? 'spin 1s linear infinite' : 'none',
                            color: 'white',
                        }} /> Auto Refresh ON</span> : <span><AutorenewIcon fontSize='small' /> Auto Refresh OFF</span>}
                    </Button>
                </div>
                <form
                    className="flex items-center gap-2 max-w-md ml-auto"
